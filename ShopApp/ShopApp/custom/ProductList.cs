...
namespace ShopApp.custom
{
    public partial class ProductList : Form
    {
        ...
        private void customButton2_Click(object sender, EventArgs e)
        {
            ...
            if (!selectStock.Text.Equals("") && selectStock.Text != null)
            {
                DataGridViewRow target = this.dataGridView1.CurrentRow;
                DataRow[] data = cartTable.Select($"C_EMAIL = '{this.email}' AND P_ID = '{this.p_id}'");
                if (data.Length > 0)
                {
                    DataRow update = data[0];
                    update["STOCK"] = int.Parse(update["STOCK"].ToString()) + int.Parse(this.selectStock.Text);
                }
                else
                {
                    DataRow newData = cartTable.NewRow();
                    newData["C_EMAIL"] = this.email;
                    newData["P_ID"] = target.Cells[0].Value.ToString();
                    newData["STOCK"] = this.selectStock.Text;
                    cartTable.Rows.Add(newData);
                }

                cartTableAdapter1.Update(dataSet11.CART);
                errorText.ForeColor = Color.MediumSeaGreen;
                errorText.Text = target.Cells[1].Value.ToString()+"이(가) 장바구니에 담겼습니다.";
            }
            else
            {
                errorText.ForeColor = Color.DarkRed;
                errorText.Text = "재고가 없습니다.";
            }
        }
